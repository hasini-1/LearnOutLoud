...
function drawFaceExpressions(canvasArg, faceExpressions, minConfidence = 0.1, textFieldAnchor) {
    ...
    faceExpressionsArray.forEach(e => {
        const expr = e instanceof faceExpressionNet_1.FaceExpressions ? e : (0, WithFaceExpressions_1.isWithFaceExpressions)(e) ? e.expressions : undefined;
        if (!expr) {
            throw new Error('drawFaceExpressions - expected faceExpressions to be FaceExpressions | WithFaceExpressions<{}> or array thereof');
        }
        const sorted = expr.asSortedArray();
        const resultsToDisplay = sorted.filter(expr => expr.probability > minConfidence);
        const anchor = (0, WithFaceDetection_1.isWithFaceDetection)(e)
            ? e.detection.box.bottomLeft
            : textFieldAnchor || new classes_1.Point(0, 0);
        const drawTextField = new DrawTextField_1.DrawTextField(resultsToDisplay.map(expr => `${expr.expression} (${(0, utils_1.round)(expr.probability)})`), anchor);
