...
class PredictAllAgeAndGenderTask extends PredictAgeAndGenderTaskBase {
    async run() {
        ...
        const ageAndGenderByFace = await (0, extractFacesAndComputeResults_1.extractAllFacesAndComputeResults)(parentResults, this.input, async (faces) => await Promise.all(faces.map(face => nets_1.nets.ageGenderNet.predictAgeAndGender(face))), this.extractedFaces);
        return parentResults.map((parentResult, i) => {
            const { age, gender, genderProbability } = ageAndGenderByFace[i];
            return (0, WithAge_1.extendWithAge)((0, WithGender_1.extendWithGender)(parentResult, gender, genderProbability), age);
        });
    }
    withFaceExpressions() {
...
class PredictSingleAgeAndGenderTask extends PredictAgeAndGenderTaskBase {
    async run() {
        ...
        const { age, gender, genderProbability } = await (0, extractFacesAndComputeResults_1.extractSingleFaceAndComputeResult)(parentResult, this.input, face => nets_1.nets.ageGenderNet.predictAgeAndGender(face), this.extractedFaces);
        return (0, WithAge_1.extendWithAge)((0, WithGender_1.extendWithGender)(parentResult, gender, genderProbability), age);
    }
    withFaceExpressions() {
