...
class ComputeAllFaceDescriptorsTask extends ComputeFaceDescriptorsTaskBase {
    async run() {
        ...
        const descriptors = await (0, extractFacesAndComputeResults_1.extractAllFacesAndComputeResults)(parentResults, this.input, faces => Promise.all(faces.map(face => nets_1.nets.faceRecognitionNet.computeFaceDescriptor(face))), null, parentResult => parentResult.landmarks.align(null, { useDlibAlignment: true }));
        return descriptors.map((descriptor, i) => (0, WithFaceDescriptor_1.extendWithFaceDescriptor)(parentResults[i], descriptor));
    }
    withFaceExpressions() {
...
class ComputeSingleFaceDescriptorTask extends ComputeFaceDescriptorsTaskBase {
    async run() {
        ...
        const descriptor = await (0, extractFacesAndComputeResults_1.extractSingleFaceAndComputeResult)(parentResult, this.input, face => nets_1.nets.faceRecognitionNet.computeFaceDescriptor(face), null, parentResult => parentResult.landmarks.align(null, { useDlibAlignment: true }));
        return (0, WithFaceDescriptor_1.extendWithFaceDescriptor)(parentResult, descriptor);
    }
    withFaceExpressions() {
