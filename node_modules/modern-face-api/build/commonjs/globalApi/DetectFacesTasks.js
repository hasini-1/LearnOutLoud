...
const MtcnnOptions_1 = require("../mtcnn/MtcnnOptions");
const SsdMobilenetv1Options_1 = require("../ssdMobilenetv1/SsdMobilenetv1Options");
...
class DetectFacesTaskBase extends ComposableTask_1.ComposableTask {
    constructor(input, options = new SsdMobilenetv1Options_1.SsdMobilenetv1Options()) {
        super();
        this.input = input;
...
class DetectAllFacesTask extends DetectFacesTaskBase {
    async run() {
        ...
        if (options instanceof MtcnnOptions_1.MtcnnOptions) {
            return (await nets_1.nets.mtcnn.forward(input, options)).map(result => result.detection);
        }
        const faceDetectionFunction = options instanceof TinyFaceDetectorOptions_1.TinyFaceDetectorOptions
            ...
            : options instanceof SsdMobilenetv1Options_1.SsdMobilenetv1Options
                ? (input) => nets_1.nets.ssdMobilenetv1.locateFaces(input, options)
                : options instanceof tinyYolov2_1.TinyYolov2Options
                    ? (input) => nets_1.nets.tinyYolov2.locateFaces(input, options)
                    : null;
    ...
    async runAndExtendWithFaceDetections() {
        const detections = await this.run();
        return detections.map(detection => (0, WithFaceDetection_1.extendWithFaceDetection)({}, detection));
    }
...
class DetectSingleFaceTask extends DetectFacesTaskBase {
    ...
    async runAndExtendWithFaceDetection() {
        const detection = await this.run();
        return detection ? (0, WithFaceDetection_1.extendWithFaceDetection)({}, detection) : undefined;
    }
    withFaceLandmarks(useTinyLandmarkNet = false) {
