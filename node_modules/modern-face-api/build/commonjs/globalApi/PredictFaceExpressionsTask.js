...
class PredictAllFaceExpressionsTask extends PredictFaceExpressionsTaskBase {
    async run() {
        ...
        const faceExpressionsByFace = await (0, extractFacesAndComputeResults_1.extractAllFacesAndComputeResults)(parentResults, this.input, async (faces) => await Promise.all(faces.map(face => nets_1.nets.faceExpressionNet.predictExpressions(face))), this.extractedFaces);
        return parentResults.map((parentResult, i) => (0, WithFaceExpressions_1.extendWithFaceExpressions)(parentResult, faceExpressionsByFace[i]));
    }
    withAgeAndGender() {
...
class PredictSingleFaceExpressionsTask extends PredictFaceExpressionsTaskBase {
    async run() {
        ...
        const faceExpressions = await (0, extractFacesAndComputeResults_1.extractSingleFaceAndComputeResult)(parentResult, this.input, face => nets_1.nets.faceExpressionNet.predictExpressions(face), this.extractedFaces);
        return (0, WithFaceExpressions_1.extendWithFaceExpressions)(parentResult, faceExpressions);
    }
    withAgeAndGender() {
