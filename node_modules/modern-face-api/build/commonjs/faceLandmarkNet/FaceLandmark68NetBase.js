...
class FaceLandmark68NetBase extends FaceProcessor_1.FaceProcessor {
    postProcess(output, inputSize, originalDimensions) {
        ...
        return tf.tidy(() => {
            const createInterleavedTensor = (fillX, fillY) => tf.reshape(tf.stack([tf.fill([68], fillX), tf.fill([68], fillY)], 1), [136]);
            ...
            const landmarkTensors = tf.div(tf.sub(tf.mul(output, tf.fill([batchSize, 136], inputSize)), tf.stack(Array.from(Array(batchSize), (_, batchIdx) => createInterleavedTensor(getPaddingX(batchIdx), getPaddingY(batchIdx))))), tf.stack(Array.from(Array(batchSize), (_, batchIdx) => createInterleavedTensor(inputDimensions[batchIdx].width, inputDimensions[batchIdx].height))));
    ...
    async detectLandmarks(input) {
        ...
        const landmarksForBatch = await Promise.all(landmarkTensors.map(async (landmarkTensor, batchIdx) => {
            ...
            const xCoords = landmarksArray.filter((_, i) => (0, utils_1.isEven)(i));
            const yCoords = landmarksArray.filter((_, i) => !(0, utils_1.isEven)(i));
            return new FaceLandmarks68_1.FaceLandmarks68(Array(68)
                .fill(0)
                .map((_, i) => new classes_1.Point(xCoords[i], yCoords[i])), {
                height: netInput.getInputHeight(batchIdx),
                width: netInput.getInputWidth(batchIdx),
            });
        }));
        landmarkTensors.forEach(t => {
            t.dispose();
        });
