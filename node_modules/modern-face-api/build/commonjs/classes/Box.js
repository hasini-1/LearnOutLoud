...
class Box {
    static isRect(rect) {
        const r = rect;
        return !!rect && [r.x, r.y, r.width, r.height].every(utils_1.isValidNumber);
    }
    static assertIsValidBox(box, callee, allowNegativeDimensions = false) {
        ...
        if (!allowNegativeDimensions && (r.width < 0 || r.height < 0)) {
            throw new Error(`${callee} - width (${r.width}) and height (${r.height}) must be positive numbers`);
        }
    }
    constructor(_box, allowNegativeDimensions = true) {
        const box = (_box || {});
        const boxRecord = box;
        const isBbox = [boxRecord.left, boxRecord.top, boxRecord.right, boxRecord.bottom].every(utils_1.isValidNumber);
        const isRect = [boxRecord.x, boxRecord.y, boxRecord.width, boxRecord.height].every(utils_1.isValidNumber);
        if (!isRect && !isBbox) {
            throw new Error(`Box.constructor - expected box to be IBoundingBox | IRect, instead have ${JSON.stringify(box)}`);
        }
        const [x, y, width, height] = isRect
            ? [boxRecord.x, boxRecord.y, boxRecord.width, boxRecord.height]
            : [
                boxRecord.left,
                boxRecord.top,
                boxRecord.right - boxRecord.left,
                boxRecord.bottom - boxRecord.top,
            ];
    ...
    rescale(s) {
        const scaleX = (0, utils_1.isDimensions)(s) ? s.width : s;
        const scaleY = (0, utils_1.isDimensions)(s) ? s.height : s;
        return new Box({
            x: this.x * scaleX,
            y: this.y * scaleY,
